...
pub fn toggle_deafen(cx: &mut App) {
    if let Some(active_call) = ActiveCall::try_global(cx)
        && let Some(room) = active_call.read(cx).room().cloned()
    {
...
impl TitleBar {
    pub(crate) fn render_collaborator_list(
        ...
        let room =
            ActiveCall::try_global(cx).and_then(|active_call| active_call.read(cx).room().cloned());
        ...
        if can_use_microphone && screen_sharing_supported {
            let trigger = IconButton::new("screen-share", IconName::Screen)
                ...
                .on_click(move |_, window, cx| {
                    let should_share = ActiveCall::try_global(cx)
                ...
                Some(ContextMenu::build(window, cx, |context_menu, _, cx| {
                    cx.spawn(async move |this: WeakEntity<ContextMenu>, cx| {
                        ...
                        this.update(cx, |this, cx| {
                            let active_screenshare_id = ActiveCall::try_global(cx)
                                .and_then(|active_call| active_call.read(cx).room().cloned())
