...
impl QuickActionBar {
    ...
    fn item_editor(item: &dyn ItemHandle, cx: &App) -> Option<Entity<Editor>> {
        item.downcast::<Editor>().or_else(|| {
    ...
    fn active_editor(&self, cx: &App) -> Option<Entity<Editor>> {
        self.active_item
            .as_deref()
            .and_then(|item| Self::item_editor(item, cx))
    }
...
impl ToolbarItemView for QuickActionBar {
    fn set_active_pane_item(
        ...
        if let Some(active_item) = active_pane_item {
            ...
            if let Some(editor) = Self::item_editor(active_item, cx) {
                let (
